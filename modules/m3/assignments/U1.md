...
Frågespelet består lämpligen av två funktioner, t.ex. `question_game` och `question`. `question_game` välkomnar användaren till spelet och ställer en fråga åt gången genom att anropa funktionen `question`. Funktionern `question` ska ta två parametrar 1) Frågan som ska ställas, 2) Det korrekta svaret. Så ett exempelanrop på funktionen `question` ska se ut såhär:
...
2. Ta emot användarens svar
	- Om svaret är korrekt så grattuleras användaren (och ni noterar att en fråga är besvarad)
...
- Ni ska ha en funktion `querstion` så har två parametrar (fråga, svar) som hanterar frågeställningen till användaren.
